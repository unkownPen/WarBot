...
from bot.utils import get_faction_status_emoji, get_all_faction_warnings
...
class UnionCommands(commands.Cog):
    ...
    def _reps(self, members):
        ...
        for m in dict.fromkeys(str(x) for x in members):
            u = self._union(m) or {}
            key = str(u.get("id")) if u.get("id") else m
            if key not in seen:
                seen.add(key)
                reps.append(m)
        return reps
    ...
    def _aggregate(self, members):
        reps = self._reps(members)
        civs = [self._civ(m) or {} for m in reps]
        if not civs:
            return {}
        out = deepcopy(civs[0])
        for field in ("resources", "military"):
            ...
            keys = set()
            for c in civs:
                keys.update((c.get(field) or {}).keys())
            for k in keys:
        ...
        if pops:
            p = deepcopy(pops[0])
            p["citizens"] = sum(x.get("citizens", 0) for x in pops)
            p["employed"] = sum(x.get("employed", 0) for x in pops)
            p["happiness"] = round(sum(x.get("happiness", 0) for x in pops) / len(pops))
            p["hunger"] = round(sum(x.get("hunger", 0) for x in pops) / len(pops))
            out["population"] = p
        for field in ("bonuses", "policies", "corporations", "megaprojects"):
            ...
            for c in civs:
                ...
                if isinstance(v, dict):
                    for k, x in v.items():
                        merged[k] = merged.get(k, 0) + x if isinstance(x, (int, float)) else deepcopy(x)
            if merged:
                out[field] = merged
    ...
    def _combine_aggregates(self, a, b, members):
        ...
        ap = (a or {}).get("population", {})
        bp = (b or {}).get("population", {})
        p = deepcopy(ap or bp)
        p["citizens"] = ap.get("citizens", 0) + bp.get("citizens", 0)
        p["employed"] = min(p["citizens"], ap.get("employed", 0) + bp.get("employed", 0))
        total = p["citizens"] or 1
        p["happiness"] = round((ap.get("happiness", 0) * ap.get("citizens", 0) + bp.get("happiness", 0) * bp.get("citizens", 0)) / total)
        p["hunger"] = round((ap.get("hunger", 0) * ap.get("citizens", 0) + bp.get("hunger", 0) * bp.get("citizens", 0)) / total)
        out["population"] = p
        for field in ("bonuses", "policies", "corporations", "megaprojects"):
            x = (a or {}).get(field, {})
            y = (b or {}).get(field, {})
            merged = deepcopy(x)
            for k, v in y.items():
                merged[k] = merged.get(k, 0) + v if isinstance(v, (int, float)) and isinstance(merged.get(k), (int, float)) else deepcopy(v)
            if merged:
                out[field] = merged
        for field in ("hyper_items", "selected_cards", "purchased_cards", "owned_territories", "black_market_history"):
            vals = []
    ...
    def _write_union(self, members, union, agg):
        for m in members:
            self._ref(m).update({
                "resources": deepcopy(agg.get("resources", {})),
                "military": deepcopy(agg.get("military", {})),
                "population": deepcopy(agg.get("population", {})),
                "territory.land_size": int(agg.get("territory", {}).get("land_size", 0)),
                "name": union.get("name", agg.get("name", "Union")),
                "union": union,
                "leaders": members,
                "last_active": now_iso(),
            })
    ...
    def _install_shared_state_hooks(self):
        ...
        def merged(uid):
            ...
            if not base or len(members) < 2:
                return base
            out = self._aggregate(members)
            u = self._union(members[0]) or {}
            out["name"] = u.get("name", out.get("name"))
            out["union"] = u
            out["leaders"] = members
            return out
        ...
        def shared_update(uid, field, changes, original):
            members = self._members(uid)
            if len(members) < 2:
                return original(uid, changes)
            current = merged(uid).get(field, {})
            for k, v in changes.items():
                if k in current and isinstance(v, (int, float)):
                    current[k] = max(0, current[k] + v)
            for m in members:
                self._ref(m).update({field: dict(current), "last_active": now_iso()})
                manager._invalidate_civ(m)
            return True

        def rh(uid, c):
            return shared_update(uid, "resources", c, original_resources)

        def ph(uid, c):
            members = self._members(uid)
            if len(members) < 2:
                return original_population(uid, c)
            ...
            for m in members:
                self._ref(m).update({"population": dict(cur), "last_active": now_iso()})
                manager._invalidate_civ(m)
            return True

        def mh(uid, c):
            return shared_update(uid, "military", c, original_military)

        def eh(uid, c):
            members = self._members(uid)
            if len(members) < 2:
                return original_employment(uid, c)
            cur = merged(uid).get("population", {})
            cur["employed"] = max(0, min(cur.get("citizens", 0), cur.get("employed", 0) + c))
            for m in members:
                self._ref(m).update({"population": dict(cur), "last_active": now_iso()})
                manager._invalidate_civ(m)
            return True
        ...
        def th(uid, c):
            members = self._members(uid)
            if len(members) < 2:
                return original_territory(uid, c)
            result = original_territory(uid, c)
        ...
        manager.get_civilization = merged
        manager.update_resources = rh
        manager.update_population = ph
        manager.update_military = mh
        manager.update_employment = eh
        manager.update_territory = th
    ...
    async def status(self, ctx):
        ...
        civ = self.civ_manager.get_civilization(uid)
        if not civ:
            return await ctx.send("❌ You need to start a civilization first! Use `.start <name>`.")

        # Union members / leaders
        members = self._members(uid) or [uid]
        names = []
        for m in members:
            try:
                names.append((await self.bot.fetch_user(int(m))).name)
            except Exception:
                names.append(m)

        r = civ.get("resources", {})
        p = civ.get("population", {})
        mi = civ.get("military", {})
        land = civ.get("territory", {}).get("land_size", self._own_land(uid))

        e = discord.Embed(
            title=f"🏛️ {civ.get('name', 'Unknown')}",
            description=(
                f"**Leaders**: {' & '.join(names)}\n"
                f"**Ideology**: {str(civ.get('ideology') or 'None').capitalize()}\n"
        ...
        e.add_field(
            name="💰 Resources",
            value=(f"🪙 Gold: {r.get('gold', 0):,}\n"
                   f"🌾 Food: {r.get('food', 0):,}\n"
                   f"🪨 Stone: {r.get('stone', 0):,}\n"
                   f"🪵 Wood: {r.get('wood', 0):,}"),
            inline=True,
        )
        e.add_field(
            name="👥 Population",
            value=(f"Citizens: {p.get('citizens', 0):,}\n"
                   f"Employed: {p.get('employed', 0):,}\n"
                   f"Happiness: {p.get('happiness', 0)}%\n"
                   f"Hunger: {p.get('hunger', 0)}%"),
            inline=True,
        )
        e.add_field(
            name="⚔️ Military",
            value=(f"Soldiers: {mi.get('soldiers', 0):,}\n"
                   f"Spies: {mi.get('spies', 0):,}\n"
                   f"Tech: {mi.get('tech_level', 1)}"),
            inline=True,
        )
        e.add_field(
            name="🗺️ Territory",
            value=f"Land: {land:,} km²\nMembers: {len(members)}",
            inline=True,
        )
        ...
        f_lines = []
        for fkey, emoji in (("military", "⚔️"), ("merchant", "💰"), ("people", "👥")):
            val = factions.get(fkey, 50)
            f_lines.append(f"{emoji} {fkey.title()}: **{val}**/100 {get_faction_status_emoji(val)}")
        e.add_field(name="🏛️ Factions", value="\n".join(f_lines), inline=False)

        # ---- Warnings ----
        warns = get_all_faction_warnings(factions)
        if warns:
            e.add_field(name="⚠️ Unrest", value="\n".join(warns), inline=False)

        # ---- Sanctions badge ----
        try:
            now = datetime.utcnow()
            active_recv = [
                s for s in (civ.get('received_sanctions') or [])
                if datetime.fromisoformat(s['expires_at']) > now
            ]
            if active_recv:
                e.add_field(
                    name="🚫 Sanctioned",
                    value=f"{len(active_recv)} active — income reduced, trade & banking blocked.",
                    inline=False,
                )
        except Exception:
            pass
        ...
        try:
            ...
            if d > 0 or l > 0:
                e.add_field(
                    name="🏦 Bank",
                    value=f"Deposits: {d:,} | Loan: {l:,} | Credit: {credit}/100",
                    inline=False,
                )
        except Exception:
            pass
    ...
    async def unite(self, ctx, member: discord.Member = None, *, new_country: str = None):
        ...
        if target in self._members(uid):
            return await ctx.send("❌ That player is already in your union.")
        ...
        await ctx.send(
            f"🤝 {member.mention}, **{ctx.author.display_name}** wants to unite with you as "
    ...
    async def accept_unite(self, ctx, request_id: str = None):
        ...
        req = snap.to_dict()
        requester = str(req.get("requester_id"))
        target = str(req.get("target_id"))
        if req.get("status") != "pending" or target != uid:
            return await ctx.send("❌ This union request is not waiting for you.")
        a = self._members(requester) or [requester]
        b = self._members(uid) or [uid]
        members = list(dict.fromkeys(a + b))
        a_agg = self._aggregate(a)
        b_agg = self._aggregate(b) if b != a else {}
        ...
        union = {
            ...
            "members": members,
            "created_at": old_union.get("created_at", now_iso()),
        }
        self._write_union(members, union, agg)
        ref.update({"status": "accepted", "accepted_at": now_iso()})
        await ctx.send(
            f"🤝 **{req['new_country']}** has expanded into a **{len(members)}-member union**! "
            f"All members now share one country, economy, military, population and territory."
        )
        # Faction bump on both sides
        for m in (requester, uid):
            try:
                self.civ_manager.apply_faction_effects(m, "ally")
            except Exception:
                pass
    ...
    async def leave_union(self, ctx):
        ...
        members = [m for m in union.get("members", []) if str(m) != uid]
        old = (self._civ(uid) or {}).get("original_union_name") or "Independent Nation"
        shared = self._aggregate(union.get("members", []))
        count = max(1, len(union.get("members", [])))
        if members:
            dr = {k: v // count for k, v in shared.get("resources", {}).items()}
            rr = {k: max(0, v - dr.get(k, 0)) for k, v in shared.get("resources", {}).items()}
            dp = dict(shared.get("population", {}))
            dp["citizens"] //= count
            dp["employed"] = min(dp.get("employed", 0), dp["citizens"])
            rp = dict(shared.get("population", {}))
            rp["citizens"] = max(0, rp.get("citizens", 0) - dp["citizens"])
            rp["employed"] = min(rp.get("employed", 0), rp["citizens"])
            self._ref(uid).update({
                "union": None,
                "name": old,
                "original_union_name": None,
                "resources": dr,
                "population": dp,
                "territory.land_size": int(self._own_land(uid)),
                "last_active": now_iso(),
            })
            ...
            for m in members:
                self._ref(m).update({
                    ...
                    "union": newu,
                    "leaders": members,
                    "territory.land_size": int(sum(self._own_land(x) for x in members)),
                    "last_active": now_iso(),
                })
                self.civ_manager._invalidate_civ(m)
        else:
    ...
    async def annex(self, ctx, *, country: str = None):
        ...
        province = next((p for p in PROVINCE_TO_SUBREGION if p.lower() == q), None) \
            or next((p for p in PROVINCE_TO_SUBREGION if q in p.lower()), None)
        if not province:
        ...
        if str(owner) == uid or str(owner) in members:
        ...
        await ctx.send(
            f"📜 **Annexation request:** {ctx.author.mention} wants **{province}** from "
            f"**{target.get('name', 'Unknown')}**.\n<@{owner}>, accept with "
            f"`.acceptannex {ref.id}` or decline with `.declineannex {ref.id}`."
        )
    ...
    async def accept_annex(self, ctx, request_id: str = None):
        ...
        if not self._civ(requester) or not self.db.conquer_territory(requester, uid, territory):
