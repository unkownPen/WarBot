...
class MilitaryCommands(commands.Cog):
    ...
    async def train_soldiers(self, ctx, unit_type: str = None, amount: int = None):
        try:
            ...
            if bonus_units > 0:
                embed.add_field(name="Bonus Units", value=f"🎉 Ideology/faction bonus added {bonus_units} extra units!", inline=True)
            if penalty_units > 0:
                embed.add_field(name="Training Issues", value=f"⚠️ Penalty lost {penalty_units} units during training", inline=True)
            await ctx.send(embed=embed)
        except Exception as e:
    ...
    async def _process_attack_victory(self, ctx, attacker_id, defender_id, attacker_civ, defender_civ, margin, level, lucky_used=False):
        try:
            ...
            if lucky_used:
                embed.add_field(name="🍀 LUCKY STRIKE", value="Your critical hit turned the tide — spoils doubled!", inline=False)
    ...
    async def add_border(self, ctx):
        try:
            ...
            self._start_cooldown(user_id, 'addborder', cooldown_seconds)
    ...
    async def remove_border(self, ctx):
        try:
            ...
            self._start_cooldown(user_id, 'removeborder', cooldown_seconds)
    ...
    async def rectract_soldiers(self, ctx, percentage: int = None):
        try:
            ...
            self._start_cooldown(user_id, 'rectract', cooldown_seconds)
    ...
    async def retrieve_soldiers(self, ctx, percentage: int = None):
        try:
            ...
            self._start_cooldown(user_id, 'retrieve', cooldown_seconds)
    ...
    async def border_info(self, ctx):
        try:
            ...
            self._start_cooldown(user_id, 'borderinfo', cooldown_seconds)
